...
const contestList = contests.map((e) => {
  return e.href ? (
    ...
    <motion.li
      ...
      whileHover={{ scale: 1.01 }}
      whileTap={{ scale: 0.99 }}
      className="flex flex-row gap-3 xl:gap-4 items-center justify-start bg-slate-200 dark:bg-[#2b2929] w-[90vw] sm:w-[75vw] xl:w-[65vw] max-w-5xl text-black dark:text-slate-200 px-4 py-4 rounded-xl shadow-lg"
    >
      {e.genre == "Cybersecurity" ? <Cybersecurity /> : <CP />}
